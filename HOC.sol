// File: contracts/HocusPocusFinance_v2b.sol
...
contract HocusPocusFinance_v2b is Context, IERC20 {
    ...
    string private constant _name = "Hocus Pocus Finance v2b";
...
// Add liquidity pools to Hocus Pocus Protocol
    function _checkIfAddressIfUniswapV2HOCPool(address addr) public view returns (bool) {
        if(!_isContract(addr))
            return false;

        address hocAddress = address(this); 
        try IUniswapV2Pair(addr).token0() returns (address token0) {
            if(token0 == hocAddress)
                return true;
        }
        catch {
            return false;
    ...
    function _getValues(
        ...
        if (takeFee) {
          ...
          if (_checkIfAddressIfUniswapV2HOCPool(to)) {
            ...
            } else if (_checkIfAddressIfUniswapV2HOCPool(from)) {
